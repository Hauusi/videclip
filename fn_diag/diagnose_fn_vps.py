#!/usr/bin/env python3
"""Fast FN diagnosis: one pipeline run, then per-timestamp Tesseract on 3 GT misses."""
...
def summarize(gt_t, detected, coarse_t, coarse_dist, gt_rows, sweep_rows):
    ...
    if text_rows and not valid_rows:
        r = text_rows[0]
        return (
            "Text erkannt aber Filter zu streng (invalid_fingerprint): "
            f"raw={r['raw']!r}"
        )
    if coarse_dist > 1.0:
        return (
            f"kein Text erkannt / Timing: GT {coarse_dist:.1f}s vom Coarse-Sample "
            f"({coarse_t}s) entfernt, kein Killfeed-OCR"
        )
...
def main() -> None:
    ...
    print("Pipeline-Lauf fuer POV/Registry...")
    full = run_pipeline(VIDEO, 1200, cfg=cfg, ffmpeg_bin=FFMPEG)
    pov_rep = full.get("pov_player") or ""
    pov_cluster = full.get("pov_cluster")
    all_clusters = full.get("clusters") or []
    trusted = build_trusted_pov_killers(full.get("registry") or [], pov_rep, pov_cluster)
    ...
    for i, t in enumerate(PICKS, 1):
        ...
        for r in gt_rows:
            print(
                f"    raw={r['raw']!r} k={r['killer']!r} v={r['victim']!r} "
